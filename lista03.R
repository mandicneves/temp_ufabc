...
giss_nasa <- giss_nasa %>%
  mutate(Peridod = cut(
    x = .$Year, 
    breaks = c(1879, 1921, 1950, 1980, 2010, 2025),
    labels = c("1880-1920", "1921-1950", "1951-1980", "1981-2010", "2011-2024")))

criar_tabela <- function(dados) {
  
  quebras <- round(seq(-0.3, 1.05, 0.05), 2)
  rotulos <- as.character(quebras)[1:27]
  
  temp <- c(rep(0, 27))
  
  for (i in 2:ncol(dados)) {
    temp <- temp + as.numeric(table(cut(x = dados[,i], breaks = quebras, labels = rotulos)))
  }
  
  freq <- data.frame(intervalos = rotulos, frequencia_abs = temp)
  freq$frequencia_pct <- round(100 * freq$frequencia_abs / sum(freq$frequencia_abs), 1)
  freq$frequencia_acum <- round(cumsum(freq$frequencia_pct))
